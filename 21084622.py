...
def clustering_GDP_CO2_2019(df_energy,df_gdp,df_co2):
    ...
    plt.figure(figsize=(8.0, 8.0))
    cm = plt.cm.get_cmap('tab10')
    plt.scatter(df_2019_gdp_co2_normalized["GDP per capita"], df_2019_gdp_co2_normalized["CO2"], 10,marker="o", cmap=cm)
    plt.scatter(xcen, ycen, 45, "k", marker="d")
...
def Renewable_co2_2019_scatter(df_2019_energy_co2,kmeans):
    ...
    ax.scatter(selected1['Renewable_energy'], selected1['CO2'], label='Cluster 1',c='#0bb4ff')
    ax.scatter(selected2['Renewable_energy'], selected2['CO2'], label='Cluster 2',c="#dc0ab4")

    #Text labels to the scatter plots 
    for i, txt in enumerate(selected1['Country Name']):
        ax.annotate(txt, (selected1['Renewable_energy'].iloc[i], selected1['CO2'].iloc[i]))
    
    for i, txt in enumerate(selected2['Country Name']):
        ax.annotate(txt, (selected2['Renewable_energy'].iloc[i], selected2['CO2'].iloc[i]))
...
df_2019_energy_co2, kmeans= clustering_Energy_CO2_2019(df_energy,df_gdp,df_co2)
