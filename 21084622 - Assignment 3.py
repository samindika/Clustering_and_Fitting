...
def read_file(file_1, file_2, file_3):
    """
    Reads three CSV files containing data for Renewable Energy consumption,
    CO2 emmision per capita, GDP per capita
    Parameters:
            file1, file2 , file 3
    Returns:
        three pandas dataframes (df_energy,df_co2,df_gdp)
    """
...
def clustering_Energy_CO2_2019(df_energy, df_gdp, df_co2):
    """
        Merge CO2 emission and Renewable energy consumption data for year 2019,
        and perform cluster analysis.
        Step1 -  correlation
        Step2 -  Shillhout score
        Step3 -  Normalizing
        Step4 - K-means clustering
        Step5 - Original scaling

        Parameters - df_energy,df_gdp,df_co2
        returns - df_2019_energy_co2, kmeans
    """
    ...
    df_2019_energy_co2 = pd.merge(
        df_energy_2019, df_co2_2019, on="Country Name", how="outer")

    # Drop NaNs' in the merged dataframe
    df_2019_energy_co2 = df_2019_energy_co2.dropna()

    # Rename the axis
    df_2019_energy_co2 = df_2019_energy_co2.rename(
        columns={"2019_x": "Renewable_energy", "2019_y": "CO2"})
    ...
    # The scatter plot confirms that this is a good choice. 
    ...
    for ncluster in range(2, 10):
        ...
        print(ncluster, skmet.silhouette_score(
            df_2019_energy_co2[['Renewable_energy', 'CO2']], labels))

    # Number of Clusters - 2

    # Step 3 - Normalize
    df_2019_energy_co2_normalized = df_2019_energy_co2[[
        "Renewable_energy", "CO2"]].copy()
    df_2019_energy_co2_normalized, df_min, df_max = ct.scaler(
        df_2019_energy_co2_normalized)
    ...
    plt.scatter(df_2019_energy_co2_normalized["Renewable_energy"],
                df_2019_energy_co2_normalized["CO2"], 10, labels, marker="o", cmap=cm)
    ...
    plt.scatter(df_2019_energy_co2["Renewable_energy"],
                df_2019_energy_co2["CO2"], c=labels, cmap="tab10")
    ...
    plt.ylabel("CO2 emission per capita")
    plt.title("Renewable energy vs Co2 emission per capita 2019- original scale")
    plt.show()

    return df_2019_energy_co2, kmeans


def clustering_Energy_CO2_1990(df_energy, df_gdp, df_co2):
    """
        Merge CO2 emission and Renewable energy consumption data for year 1990,
        and perform cluster analysis.
        Parameters - df_energy,df_gdp,df_co2
        returns - df_1990_energy_co2, kmeans
    """
    ...
    # Merge renewable energy and Co2 emission columns on Country name
    df_1990_energy_co2 = pd.merge(
        df_energy_1990, df_co2_1990, on="Country Name", how="outer")

    # Drop NaNs in the merged dataframe
    df_1990_energy_co2 = df_1990_energy_co2.dropna()

    # Rename the axis
    df_1990_energy_co2 = df_1990_energy_co2.rename(
        columns={"1990_x": "Renewable_energy", "1990_y": "CO2"})
    ...
    for ncluster in range(2, 10):
        ...
        kmeans.fit(df_1990_energy_co2[['Renewable_energy', 'CO2']])
        labels = kmeans.labels_
        ...
        # calculate the silhoutte score
        print(ncluster, skmet.silhouette_score(
            df_1990_energy_co2[['Renewable_energy', 'CO2']], labels))

    # Step 3 - Normalize

    df_1990_energy_co2_normalized = df_1990_energy_co2[[
        "Renewable_energy", "CO2"]].copy()
    df_1990_energy_co2_normalized, df_min, df_max = ct.scaler(
        df_1990_energy_co2_normalized)
    ...
    plt.figure(figsize=(8.0, 8.0))
    cm = plt.cm.get_cmap('tab10')
    plt.scatter(df_1990_energy_co2_normalized["Renewable_energy"],
                df_1990_energy_co2_normalized["CO2"], 10, labels, marker="o", cmap=cm)
    plt.scatter(xcen, ycen, 45, "k", marker="d")
    plt.xlabel("Renewable_energy")
    plt.ylabel("CO2 emission per capita")
    ...
    # Use the original dataframe
    plt.scatter(df_1990_energy_co2["Renewable_energy"],
                df_1990_energy_co2["CO2"], c=labels, cmap="tab10")
    ...
    plt.xlabel(
        "Renewable energy consumption (% of total final energy consumption)")
    plt.ylabel("CO2 Emission per capita")
    plt.title("Renewable Energy vs Co2 emission per capita 1990- original scale ")
    plt.show()

    return df_1990_energy_co2


def clustering_GDP_CO2_2019(df_energy, df_gdp, df_co2):
    """
        Merge CO2 emission and GDP for year 2019, and perform cluster analysis.
        Parameters - df_energy,df_gdp,df_co2
        returns - df_2019_gdp_co2, kmeans
    """
    ...
    df_2019_gdp_co2 = pd.merge(
        df_gdp_2019, df_co2_2019, on="Country Name", how="outer")

    # Drop NaNs in the dataframe and rename axis
    df_2019_gdp_co2 = df_2019_gdp_co2.dropna()
    df_2019_gdp_co2 = df_2019_gdp_co2.rename(
        columns={"2019_x": "GDP per capita", "2019_y": "CO2"})
    ...
    for ncluster in range(2, 10):
        ...
        # calculate the silhoutte score
        print(ncluster, skmet.silhouette_score(
            df_2019_gdp_co2[['GDP per capita', 'CO2']], labels))

    # Step 3 - Normalize

        df_2019_gdp_co2_normalized = df_2019_gdp_co2[[
            "GDP per capita", "CO2"]].copy()
        df_2019_gdp_co2_normalized, df_min, df_max = ct.scaler(
            df_2019_gdp_co2_normalized)
    ...
    plt.figure(figsize=(8.0, 8.0))
    cm = plt.cm.get_cmap('tab10')
    plt.scatter(df_2019_gdp_co2_normalized["GDP per capita"],
                df_2019_gdp_co2_normalized["CO2"], 10, marker="o", cmap=cm)
    plt.scatter(xcen, ycen, 45, "k", marker="d")
    ...
    # Use the original dataframe
    plt.scatter(df_2019_gdp_co2["GDP per capita"],
                df_2019_gdp_co2["CO2"], c=labels, cmap="tab10")
    # colour map Accent selected to increase contrast between colours
    ...
    plt.xlabel("GDP per capita")
    plt.ylabel("CO2 Emission per capita")
    plt.title("GDP vs CO2 emission 2019")
    plt.show()

   # countries = df_2019_gdp_co2[(df_2019_gdp_co2["Country Name"]=="Qatar") |  (df_2019_gdp_co2["Country Name"]=="Kuwait") ].reset_index(drop=True)
   # countries.to_excel("countries1.xlsx")

    return df_2019_gdp_co2
...
def GDP_logistic_growth():
    """
        Fitting - Use logistic function to calucalte fitting line and
    ...
    plt.figure()
    plt.title("Sweden - GDP per capita and Predictions")
    plt.xlabel("Years")
...
def Co2_logistic_growth():
    ...
    plt.figure()
    plt.title("Sweden - Co2 emission per capita and Predictions")
    plt.xlabel("Years")
    plt.ylabel("Co2 emission per capita")
    ...
    print()
    print("Co2 in")
    print("2030", round(logistic(2030, *popt), 0),
          "+/-", round(sigma, 0), "Mill.")


def Renewable_co2_2019_scatter(df_2019_energy_co2, kmeans):
    """
        Choose a sample from each cluster and represent them using a 
        scatter plot.
        Parameters : df_2019_energy_co2,kmeans

    """

    # Take a copy of dataframe
    clustersRC = df_2019_energy_co2.copy()
    clustersRC['cluster'] = kmeans.labels_
    print(clustersRC)

    cluster1 = clustersRC.loc[clustersRC['cluster'] == 0]
    cluster2 = clustersRC.loc[clustersRC['cluster'] == 1]

    # Select a sample of Cluster 1 and Cluster 2 countries
    selected1 = cluster1.sample(n=4)
    selected2 = cluster2.sample(n=4)

    # Scatter plot
    fig, ax = plt.subplots()

    ax.scatter(selected1['Renewable_energy'],
               selected1['CO2'], label='Cluster 1', c='#0bb4ff')
    ax.scatter(selected2['Renewable_energy'],
               selected2['CO2'], label='Cluster 2', c="#dc0ab4")

    # Text labels to the scatter plots
    for i, txt in enumerate(selected1['Country Name']):
        ax.annotate(
            txt, (selected1['Renewable_energy'].iloc[i], selected1['CO2'].iloc[i]))

    for i, txt in enumerate(selected2['Country Name']):
        ax.annotate(
            txt, (selected2['Renewable_energy'].iloc[i], selected2['CO2'].iloc[i]))

    plt.xlabel('Renewable Energy')
    plt.ylabel('CO2 Emissions')
    plt.title('Renewable Energy and CO2 Emissions -2019')
    plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    plt.show()


# Function calling
df_energy, df_gdp, df_co2 = read_file(
    "Renewable_energy.csv", "Gdp_percapita.csv", "Co2_emission.csv")
df_2019_energy_co2, kmeans = clustering_Energy_CO2_2019(
    df_energy, df_gdp, df_co2)
df_1990_energy_co2 = clustering_Energy_CO2_1990(df_energy, df_gdp, df_co2)
df_2019_gdp_co2 = clustering_GDP_CO2_2019(df_energy, df_gdp, df_co2)
GDP_logistic_growth()
Co2_logistic_growth()
Renewable_co2_2019_scatter(df_2019_energy_co2, kmeans)
